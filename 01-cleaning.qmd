...
-   The 2025 data was release as a PDF and HTML table, which I scraped.
- The 2026 Spring data was released on May 12, 2026 as a PDF (I saved) and an HTML table (I scraped and saved as csv.)
...
- Save the PDF and csv to the project. Sometimes they differe.
- **If it is a new season**, update the Clubs List Google Sheet is here: [rws-mls-salaries](https://docs.google.com/spreadsheets/d/1rH1pbOZYAv_ep4iCihzqrzOYBTreV2oJ014Un4TgEjw/edit?usp=sharing)
- Download the clubs list (in Building Short Club Names section)
- Add a section to import based on previous years, then add to the merge
- Check the Clean positions section for new positions to convert
...
> Working on fall 2025 update. Downloaded as a csv (but also as pdf).

Fall 2025 data was released Oct. 29th by the MLSPA. The data was scraped from the online table, though I did also download the PDF.

```{r}
s2025_imported <- read_csv(
  "data-original/2025-fall-mlsplayers.csv",
  skip = 1,
  col_names = c(
    "first_name",
    "last_name",
  ...
  mutate(across(base_salary:guaranteed_compensation, parse_number))

s2025_imported |> glimpse()
```

### 2026 data

This is the Spring release from May 12, 2026.

```{r}
s2026_imported <- read_csv(
  "data-original/2026-mlsplayers-spring.csv",
  skip = 1,
  col_names = c(
    "first_name",
    "last_name",
  ...
  add_column(year = "2026", .before = "first_name") |> 
  mutate(across(base_salary:guaranteed_compensation, parse_number))

s2026_imported |> glimpse()
```
...
non_dip_data <- s2018_imported |> 
  bind_rows(s2019_imported, s2020_imported, s2021_imported, s2022_imported, s2023_imported, s2024_imported, s2025_imported, s2026_imported)
...
-   Add them both to a [Google Sheet](https://docs.google.com/spreadsheets/d/1rH1pbOZYAv_ep4iCihzqrzOYBTreV2oJ014Un4TgEjw/edit?usp=sharing) and match them manually
...
```{r}
# Might need to re-download the clubs list if it has been updated.
...
positions <- binded |> 
  mutate(
    ...
    case_match(
      ...
      "Center Forward/Attacking Midfielder" ~ "M-F",
      "Center-back" ~ "D",
      "Center-Back" ~ "D",
      "Center Forward" ~ "F",
      "Central Midfield" ~ "M",
